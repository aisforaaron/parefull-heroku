...
module.exports = {
  ...
  getSetCache: function (name, id, cb) {
      ...
      this.getGoogleImage(name, false, function(err, imgUrl){ // second param id dev, set true when testing locally
          ...
          // self.cacheGoogleImage(imgUrl, savePath, newName, function(err, res){
          //   if(err) throw err
              ...
              if(resp.statusCode==200) {
                ...
                file.on('finish', function() {
                  ...
                  .write(localImg, function (err) {
                    ...
                    var params  = {
                      Bucket:      'parebits',
                      Body:         fs.createReadStream(localImg), // get stream obj of file
                      ...
                      if(err){
                        ...
                        superagent
                          ...
                          .end(function (err, result) {
                            ...
                            fs.unlink(localImg, function (err, res) {
                              if (err) throw err
...
*/
  ...
  // @return bool
  // @savePath includes filename, example: assets/images/bits/345ES5-mermaid.jpg
  cacheGoogleImage: function  (url, savePath, newName, cb) {
      request
        .get(url)
        .on('response', function(resp) {
          if(resp.statusCode==200) {
            // create dir if not exist - not needed now
            // if(!fs.existsSync(savePath)) {
            //   fs.mkdirSync(savePath);
            // }  
            // write file to dir
            var file = fs.createWriteStream(savePath+newName)
            resp.pipe(file)
            file.on('finish', function() {
              file.close(cb);
              return cb(null, true)
