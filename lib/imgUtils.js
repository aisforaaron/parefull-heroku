...
module.exports = {
  ...
  getGoogleApiUrl: function(name, options) {
      var googleSearch = {
        key: 'AIzaSyAgnTnNANnovJZJgdIWbg0bV9_hEy2I1IA',
        cx:  '011571087595570382339:pwevmdg2wgy',
        q:   name //'tacos' 
      }
      var gsUrl = 'https://www.googleapis.com/customsearch/v1?'
      // Apply url parameters
      var gsCount = 0
      // add googleSearch options
      for (var key in googleSearch) {
          // skip & for first one
          if(gsCount>0) gsUrl += '&'
          gsCount++
          gsUrl += key + '=' + googleSearch[key] // &key=value
      }
      // @todo add extra passed in options?
      // ...
      return gsUrl
  },
  ...
  getGoogleImage: function  (name, dev, cb) {
    ...
    if(dev==false) {
        ...
        .get(gsUrl)
        .end(function (err, result) {
            var urls = self.parseGoogleResults(result.body)
            return cb(null, urls[0])
        })
    } else { 
      var result = require('../result.json') // full response example
      var urls   = this.parseGoogleResults(result)
      return cb(null, urls[0])
    }
  },

  // @return array of accepted urls
  parseGoogleResults: function (result) {
    var items  = result.items
    var urls   = []
    for (i=0; i<items.length; i++) {
        var url        = items[i].pagemap.imageobject[0].url
        var contenturl = items[i].pagemap.imageobject[0].contenturl
        var checkUrl   = url ? url : contenturl   // what if both are null?
        if(this.checkImageMime && checkUrl)
          urls.push(checkUrl)
    }
    return urls // this may not work right in async, could just return first working url in this.check above??
  },

  // make sure passed url of image is valid mime type and ext
  // @return bool, no cb needed
  checkImageMime: function (url) {
    // console.log('checkImageMime')
